...
interface IUploadTunjangan {
  isModal: boolean;
  handleCancel(): void;
}

export default function UploadTunjangan(props: IUploadTunjangan) {
  const [user] = useLocalStorage("user");
  const dispatch = useAppDispatch()
  const queryClient = useQueryClient()

  const { mutate: doExport, isLoading: isLoadingExport } = useExport({
    name: "export-tunjangan",
    endpoint: "tunjangan-karyawans/export",
    method: "POST",
    filename: `FORMAT_UPLOAD_TUNJANGAN.xlsx`,
  ...
  return (
    <DetailModal
      {...{
        ...props,
        title: "Upload To Update Tunjangan Karyawan",
      }}
    >
      <div className="grid grid-cols-12 px-8 py-4 gap-4">
        <div className="col-span-12 bg-slate-50 px-4 py-8 rounded-md text-center text-xs ">
          ...
          <Button
            ...
            onClick={() => doExport({})}
          >
            {isLoadingExport ? "Loading..." : "FORMAT_UPLOAD_TUNJANGAN.xlsx"}
          </Button>
        </div>
        ...
        <div className="col-span-12">
          <Dropzone
            options={{
              url: `${import.meta.env.VITE_BASE_URL}tunjangan-karyawans/import`,
              acceptedFiles:
              ...
              success: (file) => {
                ...
                dispatch(
                  setNotification({
                    message: "Berhasil Mengupdate Tunjangan Karyawan",
                    status: "success",
                  })
                );
              },
