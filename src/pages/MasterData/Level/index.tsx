...
export default function Divisi() {
  ...
  const [selectedItem, setSelectedItem] = useState<DivisiEntity>();
  ...
  const { mutate } = useDelete({
    name: "divisis",
    endpoint: `divisis/${selectedItem?.id}`,
    onSuccessCallback: () => {
      dispatch(
        setNotification({
  ...
  return (
    <>
      <div className="flex flex-nowrap justify-between mt-10">
        <h2 className=" text-lg font-medium intro-y">List Level</h2>
      ...
      <div className="grid grid-cols-12 gap-6 mt-5">
        ...
        {!isLoading ? (
          <DivisiTable
            handleEdit={(divisi: DivisiEntity) => {
              setEditModal(true);
              setSelectedItem(divisi);
            }}
            handleDelete={(divisi: DivisiEntity) => {
              setDeleteModal(true);
              setSelectedItem(divisi);
            }}
            payload={payload}
      ...
      <DeleteModal
        ...
        title={selectedItem?.nama_divisi}
      />
...
interface IDivisiTable {
  payload: GetPayload<DivisiEntity> | undefined;
  handleEdit(divisi: DivisiEntity): void;
  handleDelete(divisi: DivisiEntity): void;
}

function DivisiTable({ payload, handleEdit, handleDelete }: IDivisiTable) {
  return (
        ...
        <Table.Tbody>
          ...
          {payload?.data.map((divisi, index) => (
            <Table.Tr key={index} className="intro-x bg-white">
              ...
              <Table.Td className="capitalize">{divisi.nama_divisi}</Table.Td>
              <Table.Td>
