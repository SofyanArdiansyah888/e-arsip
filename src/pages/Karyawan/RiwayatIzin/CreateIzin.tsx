...
const schema = yup
  .object({
    ...
    nama_file: yup.string().when(['tanggal_izin'],{
      is: (tanggal_izin:string) => {
        let diff = 0;
        if(tanggal_izin){
...
export function CreateIzinTelat(props: ICreateIzinTelat) {
  ...
  const tanggalWatch = watch('tanggal_izin');
  useEffect(() => {
    if(tanggalWatch){
      const tanggal = tanggalWatch.split('-');
      const days = dayjs(tanggal[1]).diff(tanggal[0],'days');
  ...
  >({
    ...
    onSuccessCallback: () => {
      dispatch(
        setNotification({
          message: "Berhasil Membuat Izin Karyawan",
          status: "success",
        })
      );
      props.handleCancel();
  ...
  return (
    ...
    >
      <div className="grid grid-cols-12 px-8 py-4 gap-2">
        {/* STATUS  */}
        ...
        {/* FILE  */}
        {isFileShow && <div className="col-span-6">
          <Dropzone
            options={{
