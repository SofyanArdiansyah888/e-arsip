...
interface IEditRiwayatTraining {
  isModal: boolean;
  handleCancel(): void;
  selectedItem: RiwayatTrainingKaryawanEntity | undefined;
}
const schema = yup
  .object({
...
export default function CreateRiwayatTraining(props: IEditRiwayatTraining) {
  ...
  useEffect(() => {
    ...
    if (props.selectedItem) {
      setValue("nama_kegiatan", selectedItem!.nama_kegiatan);
      setValue("tahun", selectedItem!.tahun);
      setValue("valid", selectedItem!.valid);
  ...
  const { mutate, isLoading } = usePut<FormData & { karyawan_id: number }>({
    endpoint: `riwayat-training-karyawans/${props.selectedItem?.id}`,
    name: "karyawan-details",
    onSuccessCallback: () => {
      dispatch(
        setNotification({
          message: "Berhasil Membuat RiwayatTraining Baru",
          status: "success",
        })
      );
      props.handleCancel();
  ...
  function handleUpdateRiwayatTraining(data: FormData) {
    if (selectedKaryawan)
      mutate({
        ...data,
  ...
  return (
    <BaseModal
      {...{
        ...props,
        title: "Edit RiwayatTraining",
        handleSubmit,
        submitCallback: handleUpdateRiwayatTraining,
        control,
        isLoading,
      }}
    >
