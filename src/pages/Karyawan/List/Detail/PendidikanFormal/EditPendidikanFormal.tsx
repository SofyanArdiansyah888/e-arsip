...
interface IEditPendidikanFormal {
  isModal: boolean;
  handleCancel(): void;
  selectedItem: PendidikanKaryawanEntity | undefined;
}
const schema = yup
  .object({
...
export default function CreatePendidikanFormal(props: IEditPendidikanFormal) {
  ...
  useEffect(() => {
    ...
    if (props.selectedItem) {
      setValue("nama_sekolah", selectedItem!.nama_sekolah);
      setValue("jenjang", selectedItem!.jenjang);
      setValue("tahun_masuk", selectedItem!.tahun_masuk);
      setValue("tahun_keluar", selectedItem!.tahun_keluar);
    }
  }, [props.isModal]);
  ...
  const { mutate, isLoading } = usePut<FormData & { karyawan_id: number }>({
    endpoint: `pendidikan-karyawans/${props.selectedItem?.id}`,
    name: "karyawan-details",
    onSuccessCallback: () => {
      dispatch(
        setNotification({
          message: "Berhasil Membuat PendidikanFormal Baru",
          status: "success",
        })
      );
      props.handleCancel();
  ...
  function handleUpdatePendidikanFormal(data: FormData) {
    if (selectedKaryawan)
      mutate({
        ...data,
  ...
  return (
    <BaseModal
      {...{
        ...props,
        title: "Edit PendidikanFormal",
        handleSubmit,
        submitCallback: handleUpdatePendidikanFormal,
        control,
        isLoading,
      }}
    >
